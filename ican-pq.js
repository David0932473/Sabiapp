...
window.onload = async () => {
    ...
    currentSubject = params.get('subject') ? params.get('subject').trim() : null;
...
async function loadData() {
    ...
    try {
        ...
        for (const tableName of tableNames) {
            // This query fetches data where subject_name matches regardless of case
            const { data: tableData, error } = await supabaseClient
                .from(tableName)
                .select('*')
                .ilike('subject_name', currentSubject);
        ...
        rawData.forEach(q => {
            let formattedQ = { text: q.question_text, insight: q.insight || "No insight available." };
            ...
            if (q.category === 'MCQ') {
                ...
                } catch(e) { console.error("MCQ Format Error", e); }
                ...
                try {
                    ...
                    // Defense against .forEach error: ensure keywords is ALWAYS an array
                    formattedQ.keywords = Array.isArray(k) ? k : [];
    ...
    if (activeSections.length > 0) {
        ...
        document.getElementById('interaction-area').innerHTML = `
            <div style="text-align:center; padding: 20px;">
                ...
                <p>Check if <b>RLS Policies</b> are enabled for your tables in Supabase.</p>
            </div>`;
    }
}
...
function render() {
    ...
    document.getElementById('progress-text').innerText = Math.round((totalDone/(totalQ || 1))*100) + '%';
}
...
function gradeEssay() {
    ...
    const q = sessionData.ESSAY[currentIndex]; 
    const txt = document.getElementById('ans').value.toLowerCase();
    ...
    if (Array.isArray(q.keywords)) {
        q.keywords.forEach(group => {
            if (Array.isArray(group) && group.some(word => txt.includes(word.toLowerCase()))) {
                matches++;
            }
        });
    }

    const totalKeywords = (Array.isArray(q.keywords) && q.keywords.length > 0) ? q.keywords.length : 1;
    const pts = (matches / totalKeywords) * 12.5; scores.ESSAY += pts;
...
function finish() {
    ...
    const max = (includeMCQ?sessionData.MCQ.length:0) + (includeSAQ?sessionData.SAQ.length:0) + (includeEssay?50:0);
    const p = max > 0 ? Math.round((total/max)*100) : 0;
    document.getElementById('interaction-area').innerHTML = `<div style="text-align:center"><h1>${p}%</h1><p>${p >= 50 ? 'PASSED' : 'FAILED'}</p><button class="action-btn" onclick="window.location.reload()">Retake</button></div>`;
